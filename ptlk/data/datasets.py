...
class Atrial(Dataset):
    ...
    def __init__(self, dataset_path, training=True, transform=None):
        ...
        self.n_points = 406  # num of sample points in each point cloud
        self.template_id = 5  # select i as the template for inference
        if training:
    ...
    def __getitem__(self, idx):
        ...
        df = pd.read_csv(path)
        df = df.sample(n=self.n_points, replace=False, random_state=np.random.randint(444))
