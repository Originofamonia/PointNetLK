...
def sinc1_dt(t):
    ...
    t2 = t ** 2
    r[s] = -t[s] / 3 * (1 - t2[s] / 10 * (
                1 - t2[s] / 28 * (1 - t2[s] / 54)))  # Taylor series O(t^8)
...
def rsinc1(t):
    ...
    t2 = t[s] ** 2
    r[s] = (((
                         31 * t2) / 42 + 7) * t2 / 60 + 1) * t2 / 6 + 1  # Taylor series O(t^8)
    r[c] = t[c] / sin(t[c])
...
def rsinc1_dt(t):
    ...
    c = (s == 0)
    t2 = t[s] ** 2
    r[s] = ((((127 * t2) / 30 + 31) * t2 / 28 + 7) * t2 / 30 + 1) * t[
        s] / 3  # Taylor series O(t^8)
...
def rsinc1_dt_csc(t):
    ...
    t2 = t[s] ** 2
    r[s] = t2 * (t2 * ((
                                   4 * t2) / 675 + 2 / 63) + 2 / 15) + 1 / 3  # Taylor series O(t^8)
    r[c] = (1 / sin(t[c]) - (t[c] * cos(t[c])) / (sin(t[c]) * sin(t[c]))) / sin(
        t[c])
...
def sinc2(t):
    ...
    t2 = t ** 2
    r[s] = 1 / 2 * (1 - t2[s] / 12 * (
                1 - t2[s] / 30 * (1 - t2[s] / 56)))  # Taylor series O(t^8)
    r[c] = (1 - cos(t[c])) / t2[c]
...
def sinc2_dt(t):
    ...
    c = (s == 0)
    t2 = t ** 2
    r[s] = -t[s] / 12 * (1 - t2[s] / 5 * (1.0 / 3 - t2[s] / 56 * (
                1.0 / 2 - t2[s] / 135)))  # Taylor series O(t^8)
...
def sinc3(t):
    ...
    t2 = t[s] ** 2
    r[s] = 1 / 6 * (1 - t2 / 20 * (
                1 - t2 / 42 * (1 - t2 / 72)))  # Taylor series O(t^8)
...
def sinc3_dt(t):
    ...
    t2 = t[s] ** 2
    r[s] = -t[s] / 60 * (1 - t2 / 21 * (
                1 - t2 / 24 * (1.0 / 2 - t2 / 165)))  # Taylor series O(t^8)
...
def sinc4(t):
    ...
    t2 = t ** 2
    r[s] = 1 / 24 * (1 - t2 / 30 * (
                1 - t2 / 56 * (1 - t2 / 90)))  # Taylor series O(t^8)
