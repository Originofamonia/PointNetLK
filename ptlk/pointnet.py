...
def mlp_layers(nch_input, nch_layers, b_shared=True, bn_momentum=0.1,
               dropout=0.0):
    """ [B, Cin, N] -> [B, Cout, N] or
        [B, Cin] -> [B, Cout]
    """
...
class MLPNet(torch.nn.Module):
    ...
    def __init__(self, nch_input, nch_layers, b_shared=True, bn_momentum=0.1,
                 dropout=0.0):
        super().__init__()
        list_layers = mlp_layers(nch_input, nch_layers, b_shared, bn_momentum,
                                 dropout)
...
class TNet(torch.nn.Module):
    ...
    def __init__(self, K):
        ...
        self.mlp1 = torch.nn.Sequential(
            *mlp_layers(K, [64, 128, 1024], b_shared=True))
        self.mlp2 = torch.nn.Sequential(
            *mlp_layers(1024, [512, 256], b_shared=False))
...
class PointNet_classifier(torch.nn.Module):
    def __init__(self, num_c, ptfeat, dim_k):
        ...
        list_layers = mlp_layers(dim_k, [512, 256], b_shared=False,
                                 bn_momentum=0.1, dropout=0.0)
